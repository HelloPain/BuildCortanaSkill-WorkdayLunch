static class YelpData
{
    ...
    public static class Table_Transactions
    {
    ...
    {
        string result = null;
        using (SqlCommand Check = new SqlCommand("SELECT[Yelp_ID]FROM" + DatabaseHeader + Table_Restaurants.Header
            + "WHERE[ID]=@id", con))
        {
            Check.Parameters.AddWithValue("@id", internalResID);
            using (SqlDataReader reader = Check.ExecuteReader())
                if (reader.Read()) result = (string)reader[0];
        }
        return result;
    }
    ...
    {
        BigInt? result =null;
        using (SqlCommand Check = new SqlCommand("SELECT"+Table_Reviews.Header+".[ID]FROM" + DatabaseHeader + Table_Reviews.Header
            +","+ DatabaseHeader + Table_Restaurants_Reviews.Header+"WHERE"+DatabaseHeader+Table_Restaurants_Reviews.Header+ ".[RestaurantID]=@resID"
            +" AND"+DatabaseHeader+Table_Reviews.Header+ ".[Yelp_ID]=@revYelpID", con))
        {
            Check.Parameters.AddWithValue("@resID", internalResID);
